...
public class MilestoneAttachmentsController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<AttachmentDto>> Attach(int projectId, int milestoneId, IFormFile file) =>
        Ok(await _attachmentsService.Add(milestoneId, file));

    [HttpDelete("{attachmentId:guid}")]
    public async Task<ActionResult> Delete(int projectId, int milestoneId, Guid attachmentId)
    {
